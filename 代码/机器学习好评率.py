# -*- coding: utf-8 -*-
"""
Created on Tue May 19 14:57:15 2020
...
path='C:/Users/dell/Desktop/论文/newdata/data'
...
tf_test_features= tfidf_vectorizer.transform(X)

predictions=svm.predict(tf_test_features)
...
def typing(con,typ):
    tlist=[]
    for i in range(0,len(con)):        
        if con[i]['travelType']==typ:
            tlist.append(con[i]['content'].replace(' ','').replace('\n', '').replace('\r', ''))
    return tlist
...
type5=[]
type6=[]
with open(r'C:\Users\dell\Desktop\论文\data\大连星海假日酒店(共8939条).json','r', encoding='utf-8') as fp:
    con=json.load(fp)

type1=typing(con,'商务出差')
type2=typing(con,'家庭亲子')
type3=typing(con,'情侣出游')
type4=typing(con,'独自旅行')
type5=typing(con,'朋友出游')
type6=typing(con,'其他')

X=text_normalize(type2)
